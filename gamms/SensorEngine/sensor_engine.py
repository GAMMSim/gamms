...
class MapSensor(ISensor):
    ...
    def sense(self, node_id: int) -> None:
        ...
        - If sensor_range is infinity, all nodes are included.
        - If fov is 2*pi, no angular filtering is applied.
        - Otherwise, only nodes within the specified arc (in radians) are returned.
        The result is stored in self._data as:
            { 'nodes': {node_id: node, ...} }
        """
